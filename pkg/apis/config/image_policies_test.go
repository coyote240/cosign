...
func TestGetAuthorities(t *testing.T) {
	...
	if got := len(c[matchedPolicy][0].Key.PublicKeys); got != 1 {
		t.Errorf("Did not get what I wanted %q, got %+v", want, got)
	}
	...
	checkPublicKey(t, inlineKeyData, c[matchedPolicy][0].Key.PublicKeys[0])
...
func checkPublicKey(t *testing.T, want string, gotKey *ecdsa.PublicKey) {
	t.Helper()
	...
	if got != want {
		t.Errorf("Did not get what I wanted %s, got %s", want, string(pemBytes))
	}
}
